...
class PairedAudioDataset(Dataset):
    def __init__(self, cover_dir, stego_dir, target_length=176400):
    ...
    def __getitem__(self, idx):
        stego_path, cover_path = self.pairs[idx]
        stego, _ = torchaudio.load(stego_path)
        cover, _ = torchaudio.load(cover_path)

        stego = stego.mean(dim=0)  # Mono
        cover = cover.mean(dim=0)

        # Pad or crop to target length (4 sec at 44.1kHz)
        stego = self._pad_or_crop(stego)
        cover = self._pad_or_crop(cover)

        return stego.unsqueeze(0), cover.unsqueeze(0)  # [1, T]

    def _pad_or_crop(self, waveform):
        if waveform.shape[-1] < self.target_length:
            return torch.nn.functional.pad(waveform, (0, self.target_length - waveform.shape[-1]))
