...
class AudioDataset(Dataset):
    def __init__(self, audio_dir, labels_file=None, transform=None, sample_len=176400):
        self.audio_dir = audio_dir
        self.transform = transform
        self.sample_len = sample_len
        self.audio_samples = []
        ...
        if labels_file:
            with open(labels_file, 'r') as f:
                for line in f:
                    ...
                    full_path = os.path.join(audio_dir, path)
                    if os.path.exists(full_path):
                        self.audio_samples.append((full_path, int(label)))
        else:
            for fname in os.listdir(audio_dir):
                if fname.endswith(".wav"):
                    full_path = os.path.join(audio_dir, fname)
                    self.audio_samples.append((full_path, -1))  # unlabeled

    def __len__(self):
        return len(self.audio_samples)

    def __getitem__(self, idx):
        file_path, label = self.audio_samples[idx]
        waveform, sr = torchaudio.load(file_path)
        waveform = waveform.mean(dim=0)  # Convert to mono

        # Ensure fixed length (pad or truncate to 176400 samples)
        if len(waveform) < self.sample_len:
            waveform = torch.nn.functional.pad(waveform, (0, self.sample_len - len(waveform)))
        elif len(waveform) > self.sample_len:
            waveform = waveform[:self.sample_len]
...
class AudioDataset(Dataset):
    def __init__(self, audio_dir, labels_file=None, transform=None):
        self.audio_dir = audio_dir
        self.transform = transform
        self.audio_files = []
        ...
        if labels_file:
            with open(labels_file, 'r') as f:
                for line in f:
                    ...
                    full_path = os.path.join(audio_dir, path)
                    if os.path.exists(full_path):
                        self.audio_files.append((full_path, int(label)))
        else:
            for fname in os.listdir(audio_dir):
                if fname.endswith(".wav"):
                    full_path = os.path.join(audio_dir, fname)
                    self.audio_files.append((full_path, -1))  # -1 as placeholder for unknown label
